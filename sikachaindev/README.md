...
1. **Add chain** — Settings → Blockchains → Add custom chain  
   - Name: `SikaChainDev` (see `anchor-chain.json`)  
...
Ghana beta env template: `cp .env.sikachaindev.gh-v1 .env.local` (synced by `sync-dev-env.sh`).
